...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: tenant, error: tenantErr } = await adminSupabase
      ...
      .select().single();
    if (tenantErr) {
      if (tenantErr.code === "23505") return NextResponse.json({ error: "הקוד הייחודי כבר תפוס — נסה אחר" }, { status: 409 });
      return NextResponse.json({ error: tenantErr.message }, { status: 500 });
    }
    await adminSupabase.from("tenant_members").insert({ tenant_id: tenant.id, user_id: user.id, role: "admin" });
    await adminSupabase.from("residents").insert({ tenant_id: tenant.id, user_id: user.id, name: user.user_metadata?.full_name ?? user.email ?? "מנהל", role: "admin", balance: 0, created_by: user.id });
